...
def text_cleaning_step(df):
            ...
            if not text_cols:
                ...
                if st.checkbox("Remove extra spaces"):
                    df[col_choice] = df[col_choice].str.strip().str.replace(r"\s+", " ", regex=True)
                    st.success(f"Extra spaces removed from '{col_choice}'")
        
                if st.checkbox("Remove special characters (keep letters/numbers/spaces)"):
                    df[col_choice] = df[col_choice].str.replace(r"[^A-Za-z0-9\s]+", "", regex=True)
            ...
            if st.button("Replace", key=f"replace_btn_{col_choice}"):
                if find_text != "":
                    df[col_choice] = df[col_choice].str.replace(find_text, replace_text, regex=False)
    ...
    if col_to_clean:
        ...
        with col3:
            if st.button("Forward Fill"):
                df[col_to_clean].fillna(method='ffill', inplace=True)
        ...
        with col4:
            if st.button("Backward Fill"):
                df[col_to_clean].fillna(method='bfill', inplace=True)
...
def outlier_cleaning_step(df):
    """Step 4: Outlier Detection and Custom Cleaning"""
...
def final_overview_and_export(df):
    ...
    log_action(f"Dataset ready for export as {filename}")
...
if st.session_state.action_log:
        with st.sidebar.expander("📝 Action Log", expanded=False):
            for action in reversed(st.session_state.action_log[-25:]):  # Show last 25 actions
                st.text(action)
...
def main():
    ...
    if st.session_state.current_step == 1:
        ...
        if uploaded_file is not None:
            try:
                ...
                st.session_state.original_df = df.copy()  # Keep original for comparison
    ...
    # Steps 2-6: Only show if data is loaded
